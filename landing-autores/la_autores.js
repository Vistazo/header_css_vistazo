...
fetch('https://header-css-vistazo.vercel.app/landing-autores/data_autores.json')
  ...
  .then(data => {
    ...
    data.forEach(autor => {
      // Mostrar el autor si la URL coincide con el link
      // const linkPath = new URL(autor.link).pathname;
      const linkPath = new URL(autor.link, window.location.origin).pathname;

      if (pathname === linkPath) {
        console.log('Autor coincidente:', autor.nombre);
      }else{
        console.log('Autor no coincidente');
        // Generar la tarjeta normalmente
